...
while [ "$n" -lt "$NETWORK_WAIT_SECONDS" ]; do
  if value="$(resolve_public_domain 2>/dev/null)"; then
    PUBLIC_DOMAIN="$value"
  fi
  ...
  if [ "$n" -eq 0 ]; then
    echo "NETWORKING_DISCOVERY=WAITING_FOR_CURRENT_RAILWAY_ENDPOINTS"
  fi
  sleep 1
  n=$((n + 1))
done
...
try:
 ...
 h=str(d.get("tcp_proxy_domain","")).strip()
 port=str(d.get("tcp_proxy_port","")).strip()
 ...
 pass' "$D/networking-snapshot.json")"
...
case "$TCP_PORT" in
  ''|*[!0-9]*) echo "FATAL: Railway TCP Proxy port must be numeric" >&2; exit 1 ;;
esac
...
if [ "$NETWORKING_SOURCE" = "current-deployment-environment" ]; then
  printf '%s\n' "{\"source\":\"current-deployment-environment\",\"authoritative\":true,\"public_domain\":\"$PUBLIC_DOMAIN\",\"tcp_proxy_domain\":\"$TCP_HOST\",\"tcp_proxy_port\":$TCP_PORT,\"application_port\":8080}" > "$D/networking-snapshot.json.tmp"
